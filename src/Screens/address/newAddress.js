...
function UserAddnewAddress() {
  ...
  const setSearchValue = details => {
    ...
    for (let i = 0; i < details.address_components.length; i++) {
      for (let j = 0; j < details.address_components[i].types.length; j++) {
        ...
        if (details.address_components[i].types[j] === 'locality') {
        ...
        if (json) {
          ...
          for (let i = 0; i < json.results[0].address_components.length; i++) {
            for (
              ...
              if (
                json.results[0].address_components[i].types[j] === 'locality'
              ) {
                setCity(json.results[0].address_components[i].long_name);
              }
            }
          }
      ...
      postWithOutToken(baseUrl, 'module/user/rest-user', {
        ...
        .then(response => {
          ...
          if (response.status === true) {
            let address_data = response.data;
            ...
            setLoader(false);
            showToastMsg('Address Add Successfully');
            ...
            showToastMsg('Something is wrong try after some time');
          }
        })
        .catch(error => {
  ...
  return (
    <View
      ...
      <ScreenHeader title="Add Address" back_display={true} />
